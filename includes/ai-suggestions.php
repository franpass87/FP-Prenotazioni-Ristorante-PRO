...
function rbf_is_restaurant_open_on_date( $date ) {
	...
	$truthy_open_values = array( 'yes', '1', 'true', 'on' ); // Match frontend truthy handling for open days
...
function rbf_ajax_get_suggestions_callback() {
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'rbf_ajax_nonce' ) ) {
		wp_send_json_error( array( 'message' => rbf_translate_string( 'Errore di sicurezza.' ) ) );
		return;
	}
	...
	if ( ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $date ) ) {
		wp_send_json_error( array( 'message' => rbf_translate_string( 'Formato data non valido.' ) ) );
		return;
	}
