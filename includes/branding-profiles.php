...
function rbf_get_brand_profiles() {
	$profiles = get_option( 'rbf_brand_profiles', array() );
...
add_action(
	'admin_post_rbf_save_brand_profile',
	function () {
		...
		wp_safe_redirect( add_query_arg( array( 'saved_profile' => $id ), wp_get_referer() ?: admin_url( 'admin.php?page=rbf_settings#branding' ) ) );
		exit;
	}
);

add_action(
	'admin_post_rbf_apply_brand_profile',
	function () {
		if ( ! current_user_can( rbf_get_settings_capability() ) ) {
			wp_die( __( 'Permesso negato.', 'rbf' ) );
		}
...
add_action(
	'admin_post_rbf_delete_brand_profile',
	function () {
		if ( ! current_user_can( rbf_get_settings_capability() ) ) {
			wp_die( __( 'Permesso negato.', 'rbf' ) );
		}
...
add_action(
	'admin_post_rbf_import_brand_profiles',
	function () {
		if ( ! current_user_can( rbf_get_settings_capability() ) ) {
			wp_die( __( 'Permesso negato.', 'rbf' ) );
		}
