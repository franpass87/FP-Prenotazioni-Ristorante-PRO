...
function rbf_enqueue_frontend_assets() {
	...
	if ( ! rbf_post_has_booking_form( $post ) ) {
		return;
	}
	...
	wp_enqueue_style( 'rbf-flatpickr-css', $flatpickr_css_src, array(), $flatpickr_version );
	wp_enqueue_script( 'rbf-flatpickr', $flatpickr_js_src, array(), $flatpickr_version, true );

	$deps = array( 'jquery', 'rbf-flatpickr' );
	...
	if ( $locale === 'it' ) {
		...
		wp_enqueue_script( 'rbf-flatpickr-locale-it', $flatpickr_locale_src, array( 'rbf-flatpickr' ), $flatpickr_version, true );
	...
	wp_enqueue_style( 'rbf-intl-tel-input-css', $intl_tel_css_src, array(), $intl_tel_version );
...
JS;
	...
	$closed_days     = array();
	$open_values     = array( 'yes', '1', 'true', 'on' );
	...
	foreach ( $active_meals as $meal ) {
		...
		$available                        = $meal['available_days'] ?? array();
	...
	wp_localize_script(
		...
		array(
			...
			'closedSingles'        => rbf_ensure_array( $closed_specific['singles'] ?? array() ),
			'closedRanges'         => rbf_ensure_array( $closed_specific['ranges'] ?? array() ),
			'exceptions'           => rbf_ensure_array( $closed_specific['exceptions'] ?? array() ),
...
function rbf_render_customer_booking_management() {
	...
	if ( isset( $_POST['cancel_booking'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'cancel_booking_' . $booking_id ) ) {
		if ( in_array( $status, array( 'confirmed' ) ) ) {
	...
	<div class="rbf-customer-management">
		...
		$can_cancel = in_array( $status, array( 'confirmed' ) ) && $booking_datetime instanceof DateTimeImmutable && $booking_datetime > $now;
...
function rbf_render_booking_form( $atts = array() ) {
	...
	if ( ! empty( $active_meals ) ) {
		foreach ( $active_meals as $meal_config ) {
			...
			$fallback_time_map[ $meal_id ] = array(
						...
						<div class="rbf-fade-in">
							<select id="rbf-time" name="rbf_orario" required aria-describedby="time-help">
								...
								<?php if ( ! empty( $fallback_time_map ) ) : ?>
									<?php
									foreach ( $fallback_time_map as $meal_id => $fallback_config ) :
										$meal_label = $fallback_config['label'] ?? $meal_id;
										$time_slots = $fallback_config['times'] ?? array();
										if ( empty( $time_slots ) ) {
											continue;
										}
										?>
								...
								></div>
									...
									<select id="rbf-phone-prefix" name="rbf_phone_prefix" aria-labelledby="rbf-phone-prefix-label">
										...
										foreach ( $phone_prefixes as $prefix ) :
											...
											?>
											<option value="<?php echo $option_prefix; ?>" data-country="<?php echo $country_code; ?>" data-label="<?php echo $option_label; ?>" 
											<?php
											if ( $option_example ) :
												?>
												data-example="<?php echo $option_example; ?>"<?php endif; ?> <?php selected( $selected || $is_default ); ?>>
...
function rbf_detect_source( $data = array() ) {
	...
	// Google Ads (paid)
	if ( $gclid || ( $utm_source === 'google' && in_array( $utm_medium, array( 'cpc', 'paid', 'ppc', 'sem' ), true ) ) ) {
		return array(
			'bucket'   => 'gads',
			'source'   => 'google',
	...
	// Meta Ads (paid)
	if ( $fbclid || ( in_array( $utm_source, array( 'facebook', 'meta', 'instagram' ), true ) && in_array( $utm_medium, array( 'cpc', 'paid', 'ppc', 'ads' ), true ) ) ) {
		return array(
			'bucket'   => 'fbads',
	...
	if ( ( strpos( $referrer, 'facebook.' ) !== false || strpos( $referrer, 'instagram.' ) !== false ) ||
		( in_array( $utm_source, array( 'facebook', 'meta', 'instagram' ), true ) && ( $utm_medium === '' || in_array( $utm_medium, array( 'social', 'organic' ), true ) ) ) ) {
		return array(
			'bucket'   => 'fborg',
			'source'   => $utm_source ?: 'facebook',
	...
	foreach ( $closed_items as $item ) {
		...
		if ( strpos( $item, '|' ) !== false ) {
			...
			if ( DateTime::createFromFormat( 'Y-m-d', $date ) !== false ) {
				...
				if ( in_array( $type, array( 'closure', 'holiday' ) ) ) {
					$singles[] = $date;
				}
			}
		}
		...
		elseif ( DateTime::createFromFormat( 'Y-m-d', $item ) !== false ) {
				$singles[] = $item;
...
function rbf_get_special_hours_for_date( $date, $meal = null, $options = null ) {
	...
	foreach ( $exceptions as $exception ) {
		if ( in_array( $exception['type'], array( 'special', 'extended' ) ) && ! empty( $exception['hours'] ) ) {
...
add_action(
	'rbf_booking_created',
	function ( $booking_id, $booking_context = array() ) {
		$date = '';
		$meal = '';
...
add_action(
	'rbf_booking_status_changed',
	function ( $booking_id, $old_status, $new_status, $note = '' ) {
		$date = get_post_meta( $booking_id, 'rbf_data', true );
		$meal = get_post_meta( $booking_id, 'rbf_meal', true ) ?: get_post_meta( $booking_id, 'rbf_orario', true );
