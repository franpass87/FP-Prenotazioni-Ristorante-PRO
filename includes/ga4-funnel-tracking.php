...
function rbf_ajax_track_ga4_event() {
	...
	$event_params_raw = $_POST['event_params'] ?? array();
	if ( is_string( $event_params_raw ) ) {
		$decoded_params = json_decode( wp_unslash( $event_params_raw ), true );
		$event_params   = is_array( $decoded_params ) ? $decoded_params : array();
	} else {
	...
	if ( ! empty( $config['api_secret'] ) ) {
		...
		if ( $result['success'] ) {
			...
			wp_send_json_error( array( 'message' => 'Failed to send to GA4: ' . $result['error'] ) );
		}
	} else {
		// Just acknowledge the client-side tracking
		wp_send_json_success( array( 'message' => 'Event logged for client-side tracking' ) );
	}
}
...
function rbf_send_ga4_measurement_protocol( $event_name, $params, $session_id, $event_id, $client_id = '', $ga_session_id = '' ) {
	...
	$event_params = is_array( $params ) ? $params : array();
...
function rbf_prepare_booking_completion_response( $booking_id, $booking_token ) {
	...
	if ( is_array( $completion_data ) && ! empty( $completion_data['event_params'] ) ) {
		...
		$event_params = is_array( $original_completion['event_params'] ) ? $original_completion['event_params'] : array();
	...
	$tracking_data = rbf_build_booking_tracking_data( $booking_id, array() );
...
function rbf_ajax_get_booking_completion_data() {
	...
	if ( $booking_token === '' ) {
		wp_send_json_error( array( 'message' => 'Missing booking token' ) );
		return;
	}
