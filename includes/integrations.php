...
function rbf_add_booking_tracking_script() {
	...
	if ( $tracking_data === null ) {
		...
		$tracking_data = rbf_build_booking_tracking_data( $booking_id, array(), $meta );
	}
...
function rbf_trigger_brevo_automation( $first_name, $last_name, $email, $date, $time, $people, $notes, $lang, $tel, $marketing, $meal ) {
	...
	$contact_response = wp_remote_post(
		...
		array_merge( $base_args, array( 'body' => wp_json_encode( $contact_payload ) ) )
	);
	...
	$event_payload = array(
		...
		'identifiers'      => array( 'email_id' => $email ),
	...
	$event_response = wp_remote_post(
		...
		array_merge( $base_args, array( 'body' => wp_json_encode( $event_payload ) ) )
	);
