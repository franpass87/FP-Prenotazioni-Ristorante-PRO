...
function rbf_print_gtm($gtm_id, $init_datalayer = true) {
    ...
    if ($init_datalayer) {
        ...
        <?php
        echo "<script>window.dataLayer.push({event:'consent', 'analytics_storage':'denied'});</script>";
        }
    ?>
    <script>
 ...
 *
 * @param bool $analytics_allowed Whether analytics storage is granted.
 */
function rbf_update_consent($analytics_allowed) {
    $analytics_status = $analytics_allowed ? 'granted' : 'denied';
    echo "<script>window.dataLayer.push({event:'consent', 'analytics_storage':'{$analytics_status}'});</script>";
}
