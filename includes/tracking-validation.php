...
function rbf_validate_tracking_setup() {
	...
	if ( $gtm_hybrid && ! empty( $gtm_id ) && ! empty( $ga4_id ) ) {
		$validation_results['hybrid_config'] = array(
			'status'  => 'ok',
	...
	if ( ! empty( $meta_pixel_id ) ) {
		if ( ! empty( $meta_access_token ) ) {
			$validation_results['facebook_tracking'] = array(
				'status'  => 'ok',
	...
	if ( ! empty( $ga4_id ) ) {
		if ( ! empty( $ga4_api_secret ) ) {
			$validation_results['ga4_server_side'] = array(
				'status'  => 'ok',
	...
	if ( ! empty( $google_ads_conversion_id ) && ! empty( $google_ads_conversion_label ) ) {
		$validation_results['google_ads_conversion'] = array(
			'status'  => 'ok',
	...
	if ( $gtm_hybrid && ! empty( $gtm_id ) && ! empty( $ga4_id ) ) {
		$validation_results['duplication_risk'] = array(
			'status'  => 'warning',
